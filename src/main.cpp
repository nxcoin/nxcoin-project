...
#if defined(NDEBUG)
# error "nxcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbb1d2cc1433e4a362504756912b18e8388f8c289fdcaf47c5c11e6d8dcb25ca2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 8); // nxcoin: starting difficulty is 1 / 2^12
...
int64 CTransaction::nMinTxFee = 1000000; // 0.01 nxcoin
/** Fees smaller than this (in ztoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 1000000; // 0.01 nxcoin
...
const string strMessageMagic = "NXCoin Signed Message:\n";
...
{
    // NXCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CBlock::CheckBlock(CValidationState &state, int nHeight, bool fCheckPOW, bool fCheckMerkleRoot, bool isVerifyDB) const
{
    ...
    // NXCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // NXCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbb1d2cc1433e4a362504756912b18e8388f8c289fdcaf47c5c11e6d8dcb25ca2");
    }
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    if ((pindexBest->nHeight+1 > 0) && (pindexBest->nHeight+1 < 210000)) {
         ...
         if(!fTestNet && (GetAdjustedTime() > nStartRewardTime)){
             FOUNDER_1_SCRIPT.SetDestination(CBitcoinAddress("aCAgTPgtYcA4EysU4UKC86EQd5cTtHtCcr").Get());
             if(pindexBest->nHeight + 1 < 9999999999){
                FOUNDER_2_SCRIPT.SetDestination(CBitcoinAddress("aLrg41sXbXZc5MyEj7dts8upZKSAtJmRDR").Get());
             }else{
                FOUNDER_2_SCRIPT.SetDestination(CBitcoinAddress("aHu897ivzmeFuLNB6956X6gyGeVNHUBRgD").Get());                
             }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NXCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NXCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("NXCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("NXCoin-Miner");
